...
import AccessibleTreeViewContext from "./AccessibleTreeView.context";
import {
...
const getClassNames = classNamesFunction<IAccessibleTreeViewStyleProps, IAccessibleTreeViewStyles>();
...
export const AccessibleTreeViewBase: React.FC<AccessibleTreeViewProps> = React.forwardRef<
	HTMLUListElement,
	AccessibleTreeViewProps
>((props, ref) => {
	const {
	...
	const [expanded, setExpanded] = useControlled(expandedProp, defaultExpanded, "AccessibleTreeView");
	const [selected, setSelected] = useControlled(selectedProp, defaultSelected, "AccessibleTreeView");
	...
	return (
		<AccessibleTreeViewContext.Provider
			value={{
				focus,
				isDisabled,
				isExpandable,
		...
		>
			<DescendantProvider>
				<ul
					ref={handleRef}
					className={classNames.AccessibleTreeView}
					id={treeId}
					onFocus={handleFocus}
